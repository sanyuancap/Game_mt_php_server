...
<body><?php
$_GET['zid']=intval($_GET['zid']);
if($_GET['zid']<1 or $_GET['zid']>5){
$_GET['zid']=1;
} 
if($_GET['chuangj']){
//exit($_GET['sex']);
...
if($_GET['sex']==0 or $_GET['sex']>2){
exit("性别错误");
} 
if($_GET['name']==""){
exit("角色名不能为空！");
} 
$alluser=$db->fetch_array($db->query("select id from ".$dbpre."player where name='".$_GET['name']."' "));
if($alluser[id]<>0){
exit("角色名已经存在");
}else{
if($data['id']==0){
$db->query("INSERT INTO `".$dbpre."player` (id,uid,name,create_time,log_ip,sex,lvl,emoney,money,til,tilsx,lingdlsx,lingdl)VALUES (null,".$user['id'].", '".$_GET['name']."','".time()."', '".$_SERVER["REMOTE_ADDR"]."',".$_GET['sex'].",1,300,25000,120,120,25,14);");
$player_id=$db->insert_id();  
give_pet($_GET['zid'],1,2);
give_pet(6,15,1);
give_pet(7,15,1);
$sql="select id from sys_taoz";
$rs=$db->query($sql);
 while($row=mysql_fetch_row($rs))   //mysql_fetch_row函数取得一行返回数组
 {
$db->query("insert into ".$dbpre."taoz(id,player_id,type)values(null,".$player_id.",".$row[0]."); ");
 }
} 
...
<form id="form1" name="form1" method="get" action="">
  <p>选择初始英雄：</p>
  <p><?php
$sql="select id,name from sys_pet where id<6 ";
$rs=$db->query($sql);
 while($row=mysql_fetch_row($rs))   //mysql_fetch_row函数取得一行返回数组
 {
echo "<a href=\"?zid=".$row[0]."&sex=".$_GET['sex']."\">".$row[1]."</a> ";
 }
?></p>
  <p>说明：<?php
$pet=$db->fetch_array($db->query("select des from sys_pet where id=".$_GET['zid']." "));
echo($pet['des']);
?></p>
  <p>性别选择：
    <?php
